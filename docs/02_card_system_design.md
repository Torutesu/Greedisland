...
MVPではカード種別ごとの実行モデルを固定する。

- Action/Item: 手札にあるカードを明示的に使用する。使用可能フェーズ、コスト、制約を解決してから効果を適用する。
- Rule/Constraint: 所持カードへ追加された時点で有効なルール層へ入る。手札から個別に使用せず、カードの `ModifyRule` を行動コンテキストへ適用する。HUDでは `Active Rule Layer` と表示する。
- Key: 直接使用せず、所持とイベント進行の条件として扱う。

`AddStatus` を含む一時的な制約カードの明示使用や、持続時間の消費処理はMVP後の拡張とする。MVPではまず、所持カードによるルールパッチと通常カード使用を同じコンテキストで検証できることを優先する。
...
MVPの基礎パーティ人数は3として扱う。`rule_party_proxy_001` が所持カード中の契約タグを人数へ加算することで、四人条件のカードを実際の実行文脈でも検証できる。

...
- [UGreeislandDebugHudWidget](/Users/torutano/Documents/Greeisland/UnrealProject/Source/Greeisland/UI/GreeislandDebugHudWidget.h)
  Blueprintで見た目を被せるだけで、初期化、イベント解決、戦闘進行、保存復元、近接イベント表示、AI GMリクエスト生成、検証、固定文面フォールバック、開発用カード付与、手札の使用不可理由表示、bootstrap 診断表示、推奨 HUD レイアウトチェックリスト取得、MVP 一周の walkthrough 取得を呼べるデバッグHUDのC++足場
